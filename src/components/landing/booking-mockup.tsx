...
import {
  ...
  MapPin, Phone, Mail, Zap, RotateCcw, MousePointerClick,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";

// ── Demo data (mirrors real station shape) ────────────────────────────────────
const STATIE = {
  id: "demo",
  nume: "Auto Test Cluj",
...
// ── Helpers (exact copy from real booking-form.tsx) ───────────────────────────
...
export function BookingMockup() {
  ...
  // Open days badges — same logic as real page
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-[#E5E7EB] shadow-xl bg-white">
      ...
      <div className="relative max-h-[620px] overflow-y-auto">
        ...
        {/* ── Real page content (1:1 with booking page) ── */}
        <div className={cn("min-h-screen bg-white", !revealed && "blur-sm pointer-events-none select-none")}>

          {/* Header — exact copy of page.tsx */}
          ...
          <main className="p-4 sm:p-6 space-y-5">
            ...
            {/* Station info — exact copy of page.tsx */}
            <div className="bg-white border border-[#E5E7EB] rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2 text-sm text-[#374151]">
                ...
                <span>{[STATIE.adresa, STATIE.oras, STATIE.judet].filter(Boolean).join(", ")}</span>
              </div>
            ...
            {/* Booking form card — exact copy of page.tsx */}
            <div className="bg-white border border-[#E5E7EB] rounded-xl p-4 sm:p-5">
              ...
              {/* BookingForm content — exact copy of booking-form.tsx */}
                  ...
                  <div className="space-y-5">
                    <div className="bg-white border border-[#E5E7EB] rounded-xl p-4">
                      <div className="flex items-center justify-between mb-4">
                        <button type="button" onClick={prevMonth} className="h-7 w-7 flex items-center justify-center rounded-md border border-[#E5E7EB] hover:bg-[#F7F8FA] transition-colors">
                          <ChevronLeft className="h-4 w-4 text-[#374151]" />
                        </button>
                        <span className="text-sm font-semibold text-[#111318]">{LUNI_RO[calMonth]} {calYear}</span>
                        <button type="button" onClick={nextMonth} className="h-7 w-7 flex items-center justify-center rounded-md border border-[#E5E7EB] hover:bg-[#F7F8FA] transition-colors">
                      ...
                      <div className="grid grid-cols-7 gap-0.5">
                        {calDays().map((date, i) => {
                          ...
                          const isClosed   = !isStationOpenOnDate(date, PROGRAM_LUCRU);
                ...
                {step === 2 && (
                  <div className="space-y-5">
                    ...
                    <div className="flex gap-3">
                      ...
                      <Button className="flex-1 bg-[#1877F2] hover:bg-[#1565D8] h-10" disabled={!step2Valid()} onClick={() => setStep(3)}>
                        Continuă<ChevronRight className="h-4 w-4 ml-1" />
                      </Button>
                    </div>
                  </div>
        ...
        {!revealed && (
          <div
            className="absolute inset-0 flex items-center justify-center z-20 bg-[#1877F2]/10 cursor-pointer"
            onClick={() => setRevealed(true)}
          >
            <div className="bg-white rounded-2xl shadow-2xl border border-[#E5E7EB] px-8 py-7 flex flex-col items-center gap-3 text-center max-w-[220px]">
              ...
              <div>
                <p className="text-sm font-bold text-[#111318] leading-snug">Demo interactiv</p>
                <p className="text-xs text-[#6B7280] mt-1 leading-relaxed">Apasă pentru a testa pagina de programări</p>
              </div>
              <span className="text-[11px] font-semibold text-white bg-[#1877F2] px-3 py-1.5 rounded-full">
                Încearcă acum
              </span>
            </div>
          </div>
...
// ── Field — exact copy from booking-form.tsx ──────────────────────────────────
...
// ── Row — exact copy from booking-form.tsx ────────────────────────────────────
