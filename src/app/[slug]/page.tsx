  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F7F8FA]">
        ...
        {/* ── FOOTER ── */}
        <footer className="border-t border-[#E5E7EB] bg-white mt-4 pb-20">
      ...
      {/* Sticky CTA bar */}
      {(statie.telefon || whatsappNr) && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#E5E7EB] px-4 py-3 flex gap-3 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]">
          {statie.telefon && (
            <a
              href={`tel:${statie.telefon}`}
              className="flex-1 flex items-center justify-center gap-2 rounded-xl border-2 py-3 text-sm font-bold transition-colors"
          ...
          {whatsappNr && (
            <a
              href={`https://wa.me/${whatsappNr.replace(/\D/g, "")}`}
              target="_blank"
              rel="noreferrer"
              className="flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold text-white transition-opacity hover:opacity-90"
              style={{ backgroundColor: "#25D366" }}
            >
